...
def get_current_user(session: SessionDep, x_windows_user: Annotated[str | None, Header(alias="X-Windows-User")] ) -> User:
    """Dependency that reads the `X-Windows-User` header injected by IIS and returns a DB User.
    ...
    """
    if not x_windows_user:
        # Not authenticated by IIS / header missing
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Missing X-Windows-User header (IIS Windows Authentication required)",
        )
